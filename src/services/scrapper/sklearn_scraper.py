# ===================================================================================
# Project: ChatSkLearn
# File: src/services/scrapper/sklearn_scraper.py
# Description: Scrape scikit-learn documentation page and extract the URLs.
# Author: LALAN KUMAR
# Created: [01-11-2025]
# Updated: [01-11-2025]
...
# Version: 1.0.0
# ===================================================================================
# CAUTION: Please run this file only when there's a update to the documentation page.(For Local uses)
...
class DocCrawlerAgent:
    def __init__(self, base_url, max_depth=5, max_pages=10000, concurrency=25):
        ...
        self.success_file = os.path.join(output_dir, "successful_urls.txt")
        ...
        try:
            ...
            async with self.semaphore:  # Limit concurrent requests
                async with session.get(url, timeout=30, headers=headers, allow_redirects=True) as response:
                    #logging.info(f"🌐 Fetch {url} → Status: {response.status}")
    ...
    async def worker(self, session, worker_id):
        while len(self.visited) < self.max_pages:
            ...
            #logging.info(f"🔍 Worker {worker_id} visiting [Depth {depth}]: {url}")
...
if __name__ == "__main__":
    ...
    logging.info(f"Crawled {len(agent.results)} URLs (failed: {len(agent.failed)})")
    logging.info(f"Average speed: {len(agent.visited) / duration:.2f} URLs/second")
    logging.info(f"Successfully crawled URLs saved to {agent.success_file}")